//this is addapted from https://android.googlesource.com/platform/bionic/+/master/tests/malloc_test.cpp
...
#define TEST(arg1, arg2) printf("starting test "#arg1",\t"#arg2"\n");

#define ASSERT(expr) do{ if (!(expr)){ printf(\
	"Test failed on condition `" \
	#expr                        \
	"` on line "                 \
	__FILE__                     \
	":%d\n",                     \
...
#define ASSERT_TRUE(arg) ASSERT((arg) == true)
#define ASSERT_LE(arg1, arg2) ASSERT((arg1) <= (arg2))
#define ASSERT_EQ(arg1, arg2) ASSERT((arg1) == (arg2))
...
// 		for (; arena != NULL; arena = arena->NextSiblingElement()) {
// 			int val;
...
// 			for (; bin != NULL; bin = bin ->NextSiblingElement()) {
...
// 	free(p);
// }
// TEST(malloc, realloc_NULL_0) {
// 	// realloc(NULL, size) is actually malloc(size).
...
// 		ASSERT_TRUE(values_ldouble[i] != NULL);
...
// 	size_t b = 2;
// 	errno = 0;
// 	ASSERT_TRUE(reallocarray(NULL, a, b) == NULL);
// 	ASSERT_EQ(ENOMEM, errno);
// 	errno = 0;
// 	ASSERT_TRUE(reallocarray(NULL, b, a) == NULL);
// 	ASSERT_EQ(ENOMEM, errno);
// #else
...
// 	errno = 0;
// 	EXPECT_EQ(false, android_mallopt(unrecognized_option, NULL, 0));
...
// 	errno = 0;
// 	if (IsDynamic()) {
// 		EXPECT_EQ(true, android_mallopt(M_INIT_ZYGOTE_CHILD_PROFILING, NULL, 0));
// 		EXPECT_EQ(0, errno);
// 	} else {
// 		// Not supported in static executables.
// 		EXPECT_EQ(false, android_mallopt(M_INIT_ZYGOTE_CHILD_PROFILING, NULL, 0));
// 		EXPECT_EQ(ENOTSUP, errno);
// 	}
...
// #if defined(__BIONIC__)
// 	EXPECT_EXIT(CheckAllocationFunction([](size_t bytes) { return calloc(bytes, 1) != NULL; }),
// 							testing::ExitedWithCode(0), "");
// 	EXPECT_EXIT(CheckAllocationFunction([](size_t bytes) { return calloc(1, bytes) != NULL; }),
// 							testing::ExitedWithCode(0), "");
// 	EXPECT_EXIT(CheckAllocationFunction([](size_t bytes) { return malloc(bytes) != NULL; }),
// 							testing::ExitedWithCode(0), "");
// 	EXPECT_EXIT(CheckAllocationFunction(
// 									[](size_t bytes) { return memalign(sizeof(void*), bytes) != NULL; }),
// 							testing::ExitedWithCode(0), "");
...
// 							testing::ExitedWithCode(0), "");
// 	EXPECT_EXIT(CheckAllocationFunction(
// 									[](size_t bytes) { return aligned_alloc(sizeof(void*), bytes) != NULL; }),
// 							testing::ExitedWithCode(0), "");
// 	EXPECT_EXIT(CheckAllocationFunction([](size_t bytes) {
// 								void* p = malloc(1024 * 1024);
// 								return realloc(p, bytes) != NULL;
// 							}),
// 							testing::ExitedWithCode(0), "");
// #if !defined(__LP64__)
// 	EXPECT_EXIT(CheckAllocationFunction([](size_t bytes) { return pvalloc(bytes) != NULL; }),
// 							testing::ExitedWithCode(0), "");
// 	EXPECT_EXIT(CheckAllocationFunction([](size_t bytes) { return valloc(bytes) != NULL; }),
// 							testing::ExitedWithCode(0), "");
// #endif
// #else
...
// 	for (size_t i = 0; i < max_pointers; i++) {
// 		ptrs[i] = malloc(kAllocationSize);
// 		ASSERT_TRUE(ptrs[i] != NULL) << "Failed to allocate on iteration " << i;
// 	}
// 	// Make sure the next allocation still fails.
// 	ASSERT_TRUE(malloc(kAllocationSize) == NULL);
// 	for (size_t i = 0; i < max_pointers; i++) {
// 		free(ptrs[i]);
// 	}
// }
...
// #if defined(__BIONIC__)
// 	if (IsDynamic()) {
// 		ASSERT_TRUE(android_mallopt(M_INIT_ZYGOTE_CHILD_PROFILING, NULL, 0));
// 	}
